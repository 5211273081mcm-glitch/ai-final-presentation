...
(function () {
  ...
  function is20MinMode() {
    if (APP.config && APP.config.presentationMode === '20m') return true;
    try {
      return new URLSearchParams(window.location.search).get('mode') === '20m';
    } catch (err) {
      return false;
    }
  }
  function coreChapterOffset() {
    return is20MinMode() ? 1 : 0;
  }
  function masterMapChapterIndex() {
    return 1 + coreChapterOffset();
  }
  function loopChapterIndex() {
    return 4 + coreChapterOffset();
  }
  function splitCfg() {
    return (C() && C().splitStages) || {};
  }
  function asset(id) {
  ...
  function initExtendedPages() {
    if (is20MinMode()) init20MinPages();
    else if (isRoadshowMode()) initRoadshowPages();
  }

  /* ═══ 20分钟 · 板块1 AI趋势 · 沙漏视频主视觉 ═══ */
  var HG_VIDEO_V = '1';

  function hgVideoSrc(file) {
    return 'assets/video/' + file + '?v=' + HG_VIDEO_V;
  }

  var HG_CHAPTER_INDEX = 0;

  function mountHourglassIfNeeded(root) {
    if (!window.HourglassHeroVideo) return;
    var stage = root.querySelector('.split-hourglass-stage');
    if (!stage) return;
    if (window.HourglassHeroVideo.isBoundTo(stage)) {
      if (window.HourglassHeroVideo.getDebugState() &&
          window.HourglassHeroVideo.getDebugState().paused) {
        window.HourglassHeroVideo.mount(stage);
      }
      return;
    }
    window.HourglassHeroVideo.mount(stage);
  ...
  function pauseHourglass() {
    if (window.HourglassHeroVideo) window.HourglassHeroVideo.pause();
  }

  function hgAccentAI(text) {
    return String(text || '').split('AI').map(esc).join('<span class="hg-kw">AI</span>');
  }

  function buildVsSplitPage() {
    var vs = splitCfg().vs || {};
    var beat0Title = vs.beat0Title || vs.topTitle || vs.leftTitle || '任务提效';
    var beat0Sub = vs.beat0Sub || vs.topSub || vs.leftSub || '用AI干活';
    var beat1Title = vs.beat1Title || vs.bottomTitle || vs.rightTitle || '流程重构';
    var beat1Sub = vs.beat1Sub || vs.bottomSub || vs.rightSub || '用AI改变干活的方法';
    var topWireTag = vs.topWireTag || '让一个动作更快';
    var bottomWireTag = vs.bottomWireTag || '让整件事情更顺';
    var webm = hgVideoSrc('hourglass-hero.webm');
    var mp4 = hgVideoSrc('hourglass-hero.mp4');
    var poster = hgVideoSrc('hourglass-hero-poster.webp');
    var videoSources =
      '<source src="' + webm + '" type="video/webm"/>' +
      '<source src="' + mp4 + '" type="video/mp4"/>';
    return '<div class="split-stage split-hourglass-stage hg-video-pending" data-beat="0">' +
      '<div class="hg-video-stage" aria-hidden="true">' +
        '<img class="hg-video-poster" src="' + poster + '" alt="" decoding="async"/>' +
        '<div class="hg-video-stack">' +
          '<video class="hg-video hg-video-a" muted playsinline webkit-playsinline preload="auto" disablePictureInPicture>' +
            videoSources +
          '</video>' +
          '<video class="hg-video hg-video-b" muted playsinline webkit-playsinline preload="auto" disablePictureInPicture>' +
            videoSources +
          '</video>' +
        '</div>' +
      '</div>' +
      '<div class="hg-vignette" aria-hidden="true"></div>' +
      '<div class="hg-text-gradient" aria-hidden="true"></div>' +
      '<div class="hg-row hg-row-top">' +
        '<div class="hg-copy hg-copy-top">' +
          '<h1 class="split-main-title hg-copy-title">' + esc(beat0Title) + '</h1>' +
          '<p class="split-sub-title hg-copy-sub">' + hgAccentAI(beat0Sub) + '</p>' +
        '</div>' +
      '</div>' +
      '<div class="hg-label-rail" aria-hidden="true">' +
        '<span class="hg-wire-tag hg-wire-tag-top">' + esc(topWireTag) + '</span>' +
        '<div class="hg-label-arrow" aria-hidden="true">' +
          '<svg class="hg-label-arrow-svg" viewBox="0 0 24 160" preserveAspectRatio="none">' +
            '<rect x="10.5" y="0" width="3" height="134" rx="1.5" fill="rgba(255,255,255,0.78)"/>' +
            '<polygon points="12,160 6,132 18,132" fill="rgba(255,255,255,0.94)"/>' +
          '</svg>' +
        '</div>' +
        '<span class="hg-wire-tag hg-wire-tag-bottom">' + esc(bottomWireTag) + '</span>' +
      '</div>' +
      '<div class="hg-row hg-row-bottom">' +
        '<div class="hg-copy hg-copy-bottom">' +
          '<h1 class="split-main-title hg-copy-title">' + esc(beat1Title) + '</h1>' +
          '<p class="split-sub-title hg-copy-sub">' + hgAccentAI(beat1Sub) + '</p>' +
        '</div>' +
      '</div>' +
    '</div>';
  }

  function applyVsSplitBeat(root, beat) {
    var stage = root.querySelector('.split-hourglass-stage');
    if (stage) stage.setAttribute('data-beat', String(beat));
  }

  /* ═══ 20分钟 · 板块3 引力进化 · 可视化面板 ═══ */
  var EVO_BRANDS = {
    '豆包': { abbr: '豆', color: '#0066FF' },
    'DeepSeek': { abbr: 'DS', color: '#0D7C8C' },
    'Kimi': { abbr: 'K', color: '#1A1A2E' },
    'GPT': { abbr: 'G', color: '#10A37F' },
    'Claude': { abbr: 'C', color: '#D97757' },
    'Gemini': { abbr: 'Ge', color: '#4285F4' },
    'Grok': { abbr: 'X', color: '#EFEFEF' },
    '即梦': { abbr: '梦', color: '#7C3AED' },
    '通义千问': { abbr: '通', color: '#6366F1' },
    '通义': { abbr: '通', color: '#6366F1' },
    '可灵': { abbr: '可', color: '#0EA5E9' },
    'Midjourney': { abbr: 'MJ', color: '#FFFFFF' },
    'Stable Diffusion': { abbr: 'SD', color: '#A855F7' },
    'SD': { abbr: 'SD', color: '#A855F7' },
    'ComfyUI': { abbr: 'Cu', color: '#F59E0B' },
    'Runway': { abbr: 'Rw', color: '#EC4899' },
    'Claude Code': { abbr: 'CC', color: '#D97757' },
    'Codex': { abbr: 'Cx', color: '#10A37F' },
    'Cursor': { abbr: 'Cr', color: '#38BDF8' },
    '通义灵码': { abbr: '灵', color: '#6366F1' }
  };

  function evoBrandChip(name) {
    var b = EVO_BRANDS[name] || { abbr: name.slice(0, 2), color: '#445566' };
    return '<span class="evo-brand" style="--bc:' + b.color + '">' +
      '<span class="evo-brand-mark">' + esc(b.abbr) + '</span>' +
      '<span class="evo-brand-label">' + esc(name) + '</span></span>';
  }

  function evoToolChipCell(brands) {
    return '<div class="evo-tool-cell">' +
      '<div class="evo-brands">' + (brands || []).map(evoBrandChip).join('') + '</div></div>';
  }

  function buildEvoToolsPanel(evo) {
    var rows = evo.toolRows || [];
    var matrixRows = rows.map(function (r) {
      return '<div class="evo-tool-matrix-row">' +
        '<div class="evo-tool-cat-label">' + esc(r.cat) + '</div>' +
        evoToolChipCell(r.domestic) +
        '<span class="evo-tool-mid" aria-hidden="true">↔</span>' +
        evoToolChipCell(r.intl) +
      '</div>';
    }).join('');
    return '<div class="evo-panel evo-visual evo-visual-tools">' +
      '<p class="split-sub-title evo-lead evo-lead-prominent">' + esc(evo.leftLead || '') + '</p>' +
      '<div class="evo-tool-matrix split-body-list">' +
        '<div class="evo-tool-matrix-head">' +
          '<div class="evo-tool-matrix-corner"></div>' +
          '<span class="evo-region-label domestic">国内</span>' +
          '<span class="evo-tool-matrix-head-mid"></span>' +
          '<span class="evo-region-label intl">海外</span>' +
        '</div>' +
        '<div class="evo-tool-matrix-body">' + matrixRows + '</div>' +
      '</div></div>';
  }

  function buildEvoMindModule(f) {
    var nodes = f.nodes || [];
    var parts = [];
    nodes.forEach(function (n, i) {
      parts.push('<li class="evo-mind-point">' +
        '<strong>' + esc(n.title) + '</strong>' +
        '<span class="evo-mind-desc">' + esc(n.desc) + '</span></li>');
      if (i < nodes.length - 1) {
        parts.push('<li class="evo-mind-arrow" aria-hidden="true">↓</li>');
      }
    });
    return '<div class="evo-mind-module">' +
      '<span class="evo-group-label">' + esc(f.label) + '</span>' +
      '<ul class="evo-mind-sublist">' + parts.join('') + '</ul></div>';
  }

  function buildEvoMindPanel(evo) {
    var modules = (evo.mindFlows || []).map(buildEvoMindModule).join('');
    return '<div class="evo-panel evo-visual evo-visual-mind">' +
      '<p class="split-sub-title evo-lead evo-lead-prominent">' + esc(evo.rightLead || '') + '</p>' +
      '<div class="evo-mind-cols">' + modules + '</div>' +
    '</div>';
  }

  function buildEvolutionPage() {
    var evo = splitCfg().evolution || {};
    var fullSlide = evo.fullSlide || {};
    var slideSrc = (fullSlide.image || 'assets/slides/feishu-multitable-demo.png') + '?v=1';
    var slideAlt = fullSlide.alt || '飞书多维表：AI落地的业务中枢';
    return '<div class="split-stage split-evo-stage" data-beat="0" style="--split:50%">' +
      '<div class="split-half split-half-left">' +
        '<div class="split-head">' +
          '<h1 class="split-main-title">' + esc(evo.leftTitle || 'AI工具') + '</h1>' +
        '</div>' +
        buildEvoToolsPanel(evo) +
      '</div>' +
      '<div class="split-gravity"><div class="split-gravity-core"></div></div>' +
      '<div class="split-half split-half-right">' +
        '<div class="split-head">' +
          '<h1 class="split-main-title">' + esc(evo.rightTitle || 'AI思维') + '</h1>' +
        '</div>' +
        buildEvoMindPanel(evo) +
      '</div>' +
      '<div class="evo-fullslide" aria-hidden="true">' +
        '<img class="evo-fullslide-img" src="' + esc(slideSrc) + '" alt="' + esc(slideAlt) + '"/>' +
      '</div>' +
    '</div>';
  }

  function applyEvolutionBeat(root, beat) {
    var stage = root.querySelector('.split-evo-stage');
    if (!stage) return;
    stage.setAttribute('data-beat', String(beat));
    if (beat === 3) return;
    if (beat === 1) stage.style.setProperty('--split', '80%');
    else if (beat === 2) stage.style.setProperty('--split', '20%');
    else stage.style.setProperty('--split', '50%');
  }

  function buildQaInteractionPage() {
    var qa = splitCfg().qa || {};
    return '<div class="split-stage qa-interact-stage" data-beat="0">' +
      '<div class="qa-interact-inner">' +
        '<h1 class="qa-interact-title">' + esc(qa.title || '提问交互') + '</h1>' +
        (qa.subtitle ? '<p class="qa-interact-sub">' + esc(qa.subtitle) + '</p>' : '') +
      '</div></div>';
  }

  function applyQaBeat(root, beat) {
    var stage = root.querySelector('.qa-interact-stage');
    if (stage) stage.setAttribute('data-beat', String(beat));
  }
  ...
  function buildLoopPage() {
    var p = C().pages[4];
    var sphereCfg = APP.config.sphere || {};
    var sc = sphereCfg.currentScene || { code: 'R-087', name: '风险场景', category: '-', alertLevel: 'P1' };
    var fallbackImg = sphereCfg.fallbackImage || 'assets/evidence/3D场景球体截图.webp';
    var sphereUrl = sphereCfg.url || '#';
    var steps = [
    ...
    return '<div class="slide-body">' +
      ...
      '<div class="loop-stage">' +
        ...
        '<div class="sphere-panel" data-sphere-panel>' +
          '<div class="sphere-box" id="sphere-box"><img class="sphere-fb" src="' + esc(fallbackImg) + '" alt="风险知识宇宙截图"></div>' +
          '<div class="scene-card"><span class="mono">' + esc(sc.code) + '</span><h3>' + esc(sc.name) + '</h3><p>类目 ' + esc(sc.category) + ' · ' + esc(sc.alertLevel) + '</p>' +
            '<a class="ev-link" href="' + esc(sphereUrl) + '" target="_blank" rel="noopener"><span>Evidence</span>204场景3D场景库演示</a>' +
          '</div>' +
        '</div>' +
      '</div>' +
  ...
  function initSphere() {
    ...
    var sphereCfg = APP.config.sphere;
    if (!sphereCfg || !sphereCfg.url) return;
    box.dataset.ok = '1';
  ...
  function initRoadshowPages() {
    if (!isRoadshowMode() || is20MinMode()) return;
  ...
  function init20MinPages() {
    PAGES = [
      { beats: 2, build: buildVsSplitPage, apply: applyVsSplitBeat },
      { beats: 2, build: buildOpening },
      { beats: 5, build: buildMasterTotalMap },
      { beats: 4, build: buildParsePage, evidence: parseEvidenceIds, apply: applyParseBeat },
      { beats: 5, build: buildHandlePage, evidence: handleEvidenceIds, apply: applyHandleBeat },
      { beats: 2, build: buildLoopPage, evidence: EVIDENCE.loop, apply: applyLoopBeat },
      { beats: 3, build: buildClosingPage },
      { beats: 4, build: buildEvolutionPage, apply: applyEvolutionBeat },
      { beats: 1, build: buildQaInteractionPage, apply: applyQaBeat }
    ];
  }
  ...
  function getNavPages() {
    if (is20MinMode() && C().pages) {
      return [{ tag: 'AI趋势' }].concat(C().pages.slice(0, 6)).concat([
        { tag: 'AI心得' },
        { tag: '提问交互' }
      ]);
    }
  ...
  function buildChapterNav() {
    ...
    $('chapter-nav').innerHTML = '<div class="cn-track">' + navPages.map(function (p, i) {
  ...
  function applyBeat(root, beat) {
    ...
    document.body.classList.toggle('on-master-map', APP.chapter === masterMapChapterIndex());
  }
  ...
  function syncHourglass(canvas, opts) {
    if (!is20MinMode() || APP.isPreview) return;
    opts = opts || {};
    if (APP.chapter === HG_CHAPTER_INDEX) {
      if (opts.beatOnly) return;
      requestAnimationFrame(function () { mountHourglassIfNeeded(canvas); });
    } else if (opts.leavingChapter0 || APP.lastChapter === HG_CHAPTER_INDEX) {
      pauseHourglass();
    }
  }
  ...
  function render() {
    ...
    APP.busy = true;
    clearMapAnimTimers();
    if (APP.chapter !== loopChapterIndex()) destroySphere();
    var canvas = $('slide-canvas');
    var chapterChanged = APP.lastChapter !== APP.chapter;
    if (is20MinMode() && chapterChanged && APP.lastChapter === HG_CHAPTER_INDEX && APP.chapter !== HG_CHAPTER_INDEX) {
      pauseHourglass();
    }
    if (APP.lastChapter !== APP.chapter) {
      ...
      setTimeout(function () {
        try {
          ...
          console.error('[final-show] render failed', err);
        } finally {
          canvas.classList.remove('fading');
          resetViewport();
          stabilizeLayout();
      ...
      applyBeat(canvas, APP.beat);
      stabilizeLayout();
      if (is20MinMode() && APP.chapter === HG_CHAPTER_INDEX) {
        syncHourglass(canvas, { beatOnly: true });
      }
      APP.busy = false;
      updateNav();
      pumpNavQueue();
  ...
  function stabilizeLayout() {
    scale();
    if (APP.chapter === loopChapterIndex()) fitSphereBox();
    requestAnimationFrame(function () {
      resetViewport();
      scale();
      if (APP.chapter === loopChapterIndex()) fitSphereBox();
  ...
  function preloadChapter(ch) {
    var pg = PAGES[ch];
    if (!pg) return;
    if (ch === masterMapChapterIndex() || ch === loopChapterIndex()) {
  ...
  function mountSphereIframe(box) {
    ...
    var sphereUrl = APP.config.sphere && APP.config.sphere.url;
    if (!sphereUrl) return;
    box.classList.add('is-loading');
    var iframe = document.createElement('iframe');
    iframe.src = sphereUrl;
    iframe.title = '风险场景库';
  ...
  function bindKeys() {
    ...
    document.addEventListener('keydown', function (e) {
      ...
      switch (e.key) {
        ...
        case 'r':
        case 'R':
          if (APP.chapter === masterMapChapterIndex()) {
            e.preventDefault();
        ...
        case 'L':
          ...
          break;
        default:
          if (e.key >= '1' && e.key <= (is20MinMode() ? '9' : isRoadshowMode() ? '7' : '6')) {
            e.preventDefault();
            goChapter(+e.key - 1, 0);
          }
      }
